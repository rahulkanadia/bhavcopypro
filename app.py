...
class ArchiverApp(App):
        ...
        with Horizontal(id="main_container"):
            with Vertical(id="left_pane", classes="pane"):
                with Vertical(id="tree_zone"):
                    yield Label("[b]Select reports to process into Database...[/b]\n(Unchecked files will still download to local archive)")
            ...
            with Vertical(id="right_pane", classes="pane"):
                with Vertical(id="form_zone"):
                    yield Label("[i]Configure dates below and start the file pull.[/i]\n", classes="row")
                    ...
                    with Horizontal(classes="row"):
                        yield Label("Specific Dates:", classes="label")
    ...
    def _update_select_all_button(self):
        ...
        btn.label = "Unselect All" if checked == total else "Select All"
    ...
    def dlog(self, ui_markup: str, raw_text: str, is_error: bool = False):
        ...
        try:
            if threading.get_ident() == self._thread_id: write_ui()
            else: self.call_from_thread(write_ui)
        except RuntimeError:
            write_ui()
    ...
    @work(thread=True)
    def run_pipeline(self, root_dir, dates, ui_selected_reports, force, unzip):
        ...
        for target_date in dates:
            if not self.pipeline_active:
                self.dlog("[bold red]Pipeline Interrupted.[/bold red]", "Pipeline Interrupted.", is_error=True)
                return

            self.dlog(f"\n[bold cyan]--- Processing Date: {target_date} ---[/bold cyan]", f"--- Processing Date: {target_date} ---")
            
            # Using a single generic ID for ledger tracking since we pull the whole archive
            rpt_id = "nse_master_archive"

            if not force and check_exists(ledger, target_date, rpt_id):
                self.dlog(f"[dim]SKIP:[/dim] Archive for {target_date} already exists.", f"SKIP: {target_date} archive exists.")
            else:
                try:
                    self.dlog("FETCHING: Requesting Master Archive via browser automation...", f"FETCHING: {target_date} Archive")
                    
                    # 1. Download the single master ZIP
                    raw_archive_path, target_sub = fetcher.fetch_archive(target_date, root_dir)
                    d_obj = datetime.strptime(target_date, "%Y-%m-%d")
                    
                    self.dlog(f"[green]DOWNLOADED:[/green] Saved to {raw_archive_path}", f"DOWNLOADED: {raw_archive_path}")

                    # 2. Unzip the contents into a temporary holding pattern and rename them
                    extracted_files = process_downloaded_file(raw_archive_path, target_sub, d_obj, "NSE", "Capital_Market", unzip)

                    # 3. Load UI-Selected Files to Database
                    if unzip:
                        for ex_file in extracted_files:
                            if ex_file.endswith(('.csv', '.txt', '.DAT', '.csv.gz')):
                                base_name = os.path.basename(ex_file)
                                try:
                                    if "sec_bhavdata_full" in base_name:
                                        table_ref = "sec_bhavdata_full"
                                    else:
                                        table_ref = base_name.split("_Capital_Market_")[-1].rsplit(".", 1)[0].lower()
                                except IndexError:
                                    table_ref = base_name.rsplit(".", 1)[0].lower()
                                
                                # We map the UI selection to the database table ref to ensure only checked reports load
                                is_selected_in_ui = any(table_ref in report_id.lower() or report_id.lower() in table_ref for report_id in ui_selected_reports)

                                if is_selected_in_ui:
                                    success, msg = load_to_postgres(ex_file, table_ref, target_date)
                                    if success:
                                        self.dlog(f"[green]DB OK:[/green] {table_ref} -> {msg}", f"DB OK: {table_ref} -> {msg}")
                                    elif "explicitly discarded" not in msg:
                                        self.dlog(f"[yellow]DB WARN:[/yellow] {table_ref} -> {msg}", f"DB WARN: {table_ref} -> {msg}", is_error=True)

                    # 4. Clean up the extracted files, leaving ONLY the master ZIP
                    if unzip:
                        for ex_file in extracted_files:
                            # Safeguard: never delete the .zip file itself
                            if os.path.exists(ex_file) and not ex_file.lower().endswith('.zip'):
                                try:
                                    os.remove(ex_file)
                                except OSError as e:
                                    self.dlog(f"[red]CLEANUP ERROR:[/red] {str(e)}", f"CLEANUP ERROR: {str(e)}", is_error=True)

                    record_download(target_date, rpt_id, "SUCCESS", raw_archive_path)

                except Exception as e:
                    self.dlog(f"[red]FAILED:[/red] {str(e)}", f"FAILED: {target_date} -> {str(e)}", is_error=True)
        ...
        self.dlog("\n[bold green]Pipeline Run Complete.[/bold green]", "Pipeline Run Complete.", is_error=True)
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "btn_start":
            ...
            ui_selected_reports = self.get_selected_reports()

            try: dates = self.parse_dates()
            except ValueError as ve:
                if str(ve) == "MISSING_DATES":
                    self.dlog("[bold red]ERROR: No dates provided.[/bold red]", "ERROR: No dates provided.", is_error=True)
                else:
            ...
            self.run_pipeline(root, dates, ui_selected_reports, force, unzip)

        elif event.button.id == "btn_select_all":
            all_checkboxes = self.query(Checkbox).exclude("#chk_force").exclude("#chk_unzip")
            new_state = (event.button.label == "Select All")
            for chk in all_checkboxes: chk.value = new_state
            self._update_select_all_button()
